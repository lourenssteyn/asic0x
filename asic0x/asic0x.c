...
static int asic0x_bind(struct usbnet *dev, struct usb_interface *intf) {
	...
	status = usbnet2_get_endpoints(dev, intf);
...
static const struct driver_info asic0x_info = {
	...
	.flags		    = FLAG_ETHER | FLAG_RX_ASSEMBLE, /* FLAG_RX_ASSEMBLE for the shot 8 byte packets returned. Won't update error counter */
	.bind		    = asic0x_bind,
